...
class PIDLockedException(Exception):
    def __init__(self, name, pid):
        Exception.__init__(
            self, '"{}" is already locked with PID: {}'.format(name, pid)
        )
...
class PIDLock:
    ...
    def __init__(self, lockdir="~/.pidlock", verbose=True):
        ...
        if not path.exists(self.lockdir):
    ...
    def lock(self, name, wait=0, mininterval=1):
        ...
        Example:
            ...
            >>>     time.sleep(10)
        """
        lockdir = path.join(self.lockdir, name + "@" + getfqdn())
        pidfile = path.join(lockdir, "lock.pid")
        pid = os.getpid()

        if path.exists(lockdir):
            # Stript is probably running
            ...
            if xpid in pids:
                if self.verbose:
                    print("PID file is locked with PID:", xpid, file=sys.stderr)
            ...
            # Else remove lock
                ...
                if self.verbose:
                    print("Removing old pidfile...", file=sys.stderr)
            ...
            # Else create lock
            if self.verbose:
                print('Locking "{}" with PID: {}'.format(name, pid), file=sys.stderr)
        ...
        try:
            ...
            os.rmdir(lockdir)
            if self.verbose:
                print("Released lock...", file=sys.stderr)
...
def pidlock_cli():
    ...
    try:
        with locker.lock(
            ...
            if parsed.verbose:
                print("Running command:", parsed.command, file=sys.stderr)
